# Objetivo: Listar servico em execucao nas maquinas.
...
##substitua o(s) nome(s) do serviço(s) olhando no comando Get-Service para obter essa informação.
Invoke-Command -ComputerName $online {  hostname; Get-Service -Name Browser, Fax   | Format-Table -AutoSize } -Credential $credencial
timeout 200
